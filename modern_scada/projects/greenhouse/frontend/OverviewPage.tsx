    ...
    <DetailCard title={`Rack ${tank.rackId}`} subTitle="Local Nutrient Supply" onClose={clearSelection}>
      <div className="space-y-4">
        ...
        <button
          ...
          disabled={tank.level > 1 || tank.status === 'FILLING'}
...
const OverviewPage = () => {
  // @ts-ignore
  const { initSystem, sensors, devices, toggleDevice, clearSelection, rackTanks, layoutConfig } = useAppStore();

  useEffect(() => { const cleanup = initSystem(); return cleanup; }, [initSystem]);

  // Construct mapping from layoutConfig (assuming mesh node names match IDs)
  const visualizationMapping = React.useMemo(() => {
    ...
    const addItems = (items: any[] | undefined) => {
      if (!items) return;
      items.forEach(item => {
        if (item.id) mapping[item.id] = item.id;
      });
    };
  ...
  return (
    <div className="w-full h-screen relative overflow-hidden bg-slate-950">
      ...
      <div className="absolute inset-0 z-0">
        <Generic3DViewer
          modelUrl="/models/greenhouse.glb"
